...
import 'package:shared_preferences/shared_preferences.dart'; // <-- Make sure this is added!
import 'dart:convert';
import 'dart:async';
import 'login.dart';
import 'newvehicle.dart'; // Links your AddVehicleScreen module

void main() async {
  WidgetsFlutterBinding.ensureInitialized();
  
  // Fetch the user preference check before the UI boots
  final SharedPreferences prefs = await SharedPreferences.getInstance();
  final bool isLoggedIn = prefs.getBool('isLoggedIn') ?? false;

  // ONLY CALL RUNAPP ONCE. Pass the logged-in status down.
  runApp(SmartParkingApp(isLoggedIn: isLoggedIn));
}

class SmartParkingApp extends StatelessWidget {
  final bool isLoggedIn;
  
  // Accept the authentication state via the constructor
  const SmartParkingApp({super.key, required this.isLoggedIn});
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      // Otherwise, hit them with the LoginScreen first.
      home: isLoggedIn ? const ParkingDashboard() : const LoginScreen(),
    );
  }
}
