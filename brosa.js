...
const PORT = process.env.PORT || 2001; // Port 2001 strictly enforced across architectures[cite: 1, 2]
...
// Intact property preserved from brosa.js[cite: 1]
...
// Standalone 8 parking spots array imported from hacathon.js
...
// 1. Unified Dashboard Overview[cite: 2]
...
// 3. Get entire architecture status for a targeted facility[cite: 1]
...
// 4. SECURE ENDPOINT: Commit a booking reservation[cite: 1, 2]
...
// 6. IoT Hardware simulation endpoint (ESP32 IR Sensor integration)[cite: 2]
app.post('/api/parking/:facilityId/toggle', (req, res) => {
  const { facilityId } = req.params;
  const { index, isFree } = req.body;

  const targetFacility = facilitiesData[facilityId];
  if (!targetFacility) return res.status(444).json({ success: false, message: "Facility not found." });
  if (index === undefined || index < 0 || index >= targetFacility.length) {
    return res.status(400).json({ success: false, message: "Invalid spot index." });
  }
  ...
  if (isFree) {
    ...
    targetFacility[index] = {
      ...
      isFree: false,
      entryTime: entryTime.toISOString(),
      endTime: new Date(entryTime.getTime() + 60 * 60000).toISOString(), 
...
io.on('connection', (socket) => {
  ...
  socket.emit('initialState', baselineSnapshot); //[cite: 2]
